...
const Inventory: React.FC = () => {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        setFormData((prev) => ({
            ...prev,
            [name]: name === 'weight' || name === 'price' || name === 'warehouseId'
                ? (value === '' ? (name === 'warehouseId' ? undefined : 0) : parseFloat(value))
                : value,
        }));
    };
    ...
    const handleEdit = (product: Product) => {
        ...
        setOriginalWeights({ weight: product.weight, remaining: product.remainingWeight });
        setWeightAdjustment(0);
                ...
                <form onSubmit={handleSubmit} className="product-form">
                    <div className="form-grid">
                        ...
                        {originalWeights.remaining < originalWeights.weight && (
                            <div className="form-group">
                                <label className="form-label" style={{ color: '#ef4444' }}>Adjust Weight (+/-)</label>
                                <input
                                    type="number"
                                    name="adjustment"
                                    step="0.01"
                                    ...
                                    onChange={(e) => setWeightAdjustment(e.target.value === '' ? 0 : parseFloat(e.target.value))}
                                    placeholder="Add (+) or Reduce (-)"
                                />
                                <div style={{ marginTop: '8px', fontSize: '14px' }}>
                                    <span style={{ marginRight: '16px' }}>
                                        New Total: <strong>{(originalWeights.weight + weightAdjustment).toFixed(2)}</strong>
                                    </span>
                                    <span>
                                        New Remaining: <strong>{(originalWeights.remaining + weightAdjustment).toFixed(2)}</strong>
                                    </span>
                                </div>
                            </div>
                        )}
