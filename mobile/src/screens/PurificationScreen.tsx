...
const PurificationScreen: React.FC = () => {
  ...
  const [records, setRecords] = useState<PurifiedRecord[]>([]);
  ...
  const [assignPurifierId, setAssignPurifierId] = useState<number>(0);
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [avail, procs, recs, purifiersData] = await Promise.all([
      ...
      setProcesses(procs);
      setRecords(recs);
      setPurifiers(purifiersData);
    } catch (e) {
      console.error(e);
      Alert.alert("Error", "Failed to load Purification data.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleOpenAssign = (category: AvailableCategory) => {
    setSelectedCategory(category);
    setAssignPurifierId(0);
  ...
  const handleAssign = async () => {
    ...
    if (!assignPurifierId) {
      Alert.alert("Validation", "Please select a purifier.");
      return;
    }

    try {
      const countVal = parseInt(assignCount) || 0;
      const feesVal = parseFloat(workerFees) || 0;
      await purificationAPI.create({
        ...
        purifyCount: countVal,
        isWeightFull: true,
        workerFees: feesVal,
        supervisorFees: 0,
        workers: [{
          purifierId: assignPurifierId,
          count: countVal,
          workerFees: feesVal,
        }],
      });
      ...
      loadData();
      Alert.alert("Success", "Category assigned to purification.");
    } catch (e: any) {
      console.error(e);
      ...
      <View style={styles.container}>
        <View style={styles.tabContainer}>
          ...
          >
            <Text style={[styles.tabText, activeSubTab === "stock" && styles.tabTextActive]}>
              Purified ({filteredRecords.length})
            </Text>
          </TouchableOpacity>
        </View>
          ...
          <View style={styles.searchContainer}>
            <View style={styles.searchWrapper}>
              ...
              <TextInput
                ...
                placeholder="Search available sorted bundles..."
                value={searchTerm}
        ...
        {loading ? (
          ...
          <FlatList
            ...
            keyExtractor={(item) => `${item.processingRecordId}-${item.category}`}
            ...
            renderItem={({ item }) => (
              <View style={styles.card}>
                ...
                >
                  ...
                  <Text style={styles.actionBtnText}>Purify</Text>
                </TouchableOpacity>
              </View>
            )}
            ListEmptyComponent={
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyText}>No bundles available for purification</Text>
              </View>
            }
          />
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={styles.card}>
                <View style={styles.cardHeader}>
                  <Text style={styles.markerText}>{item.productMarker}</Text>
                  <Text style={styles.weightText}>{item.purifyWeight.toFixed(3)} viss</Text>
                </View>
                <View style={styles.infoRow}>
                  <User size={14} color="#64748b" />
                  <Text style={styles.infoText}>Supervisor: {item.supervisorName || "N/A"}</Text>
                </View>
                <View style={styles.infoRow}>
            ...
            ListEmptyComponent={
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyText}>No ongoing purification processes</Text>
              </View>
            }
          />
        ) : (
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.emptyState}>
                <Filter size={40} color="#cbd5e1" />
                <Text style={styles.emptyText}>No purified stock records found</Text>
              </View>
            }
          />
        )}
        ...
        >
          {selectedCategory && (
            <View>
              ...
              <Text style={styles.selectLabel}>Select Purifier</Text>
              <ScrollView style={styles.workerDropdown} nestedScrollEnabled>
                {purifiers.map((w) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.workerOption,
                      assignPurifierId === w.id && styles.workerOptionActive,
                    ]}
                    onPress={() => setAssignPurifierId(w.id)}
                  >
                    <Text
                      style={[
                        styles.workerOptionText,
                        assignPurifierId === w.id && styles.workerOptionTextActive,
                      ]}
                    >
                      {w.name}
                    </Text>
