...
let apiBaseUrl = "http://localhost:5159/api"; // Default for Android emulator. Update for your local network.
...
export const workersAPI = {
  ...
  getSingleDoubleDrawnWorkers: async (): Promise<SingleDoubleDrawnWorker[]> => {
    const response = await api.get<SingleDoubleDrawnWorker[]>(
      "/workers/singledoubledrawn",
    );
    return response.data;
  },
  getSemiExportPurchaseWorkers: async (): Promise<
    SingleDoubleDrawnWorker[]
  > => {
    const response = await api.get<SingleDoubleDrawnWorker[]>(
      "/workers/semiexportpurchase",
    );
    return response.data;
  },
...
export const reportsAPI = {
  getMarkersByDate: async (
    fromDate: string,
    toDate: string,
  ): Promise<MarkerByDateDto[]> => {
    const response = await api.get<MarkerByDateDto[]>(
      "/reports/markers-by-date",
      {
        params: { fromDate, toDate },
      },
    );
...
export const cashFlowAPI = {
  ...
  getBreakdown: async (
    ...
    const response = await api.get<any[]>("/cashFlow/breakdown", {
      params: { workerId, purifierId, workerName },
    });
    return response.data;
  },
